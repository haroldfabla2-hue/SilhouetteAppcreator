    ...
    async def store_insight(self, insight: str, metadata: Optional[Dict[str, Any]] = None, conversation_id: str = "global") -> Dict[str, Any]:
        ...
        try:
            ...
            if self.vector_store and hasattr(self.vector_store, 'engine'):
                with self.vector_store.engine.connect() as conn:
                    # En Postgres con pgvector, se formatea la lista a string "[1.2, 0.5, ...]"
                    emb_str = f"[{','.join(map(str, embedding))}]"
                    conn.execute(text("""
                        INSERT INTO agent_memories (conversation_id, content, embedding, metadata)
                        VALUES (:conv_id, :content, :emb::vector, :metadata)
                    """), {
