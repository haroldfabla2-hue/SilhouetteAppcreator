    ...
    async def _code_evaluation(
        ...
        if "def " in code_snippet or "class " in code_snippet or "import " in code_snippet:
            ...
            try:
                ...
                scores["execution_test"] = {"score": 0.0, "passed": False, "justification": "Execution timed out"}
                if 'temp_path' in locals() and os.path.exists(temp_path):
                    os.unlink(temp_path)
            except Exception as e:
                scores["execution_test"] = {"score": 0.0, "passed": False, "justification": f"Test harness error: {str(e)}"}
        else:
            scores["no_errors"] = {"score": 1.0, "passed": True}
