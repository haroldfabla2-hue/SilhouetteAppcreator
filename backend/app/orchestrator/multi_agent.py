            ...
            try:
                async with self.db_pool.acquire() as conn:
                    orphans = await conn.fetch(
                        "SELECT conversation_id, status FROM agent_tasks WHERE status = 'RUNNING'"
                    )
            except Exception as e:
                logger.error(f"Error en auto-healing PG: {e}")
        else:
            ...
            if os.path.exists(self.fallback_db_path):
                try:
                    ...
                    orphans = [
                        ...
                        for k, v in data.items() if v["status"] == "RUNNING"
                    ]
                except Exception as e:
    ...
    async def _update_task_state(self, conversation_id: str, status: str, payload: Dict = None):
        ...
        if self.db_pool:
            try:
                ...
                logger.error(f"DB Error update state {conversation_id}: {e}")
        else:
            ...
            try:
                ...
                logger.error(f"JSON Fallback Error update state {conversation_id}: {e}")
    ...
    async def process_request(
        ...
        try:
            ...
            if planning_result.get("requires_human_approval", False) or "DROP TABLE" in str(planning_result).upper():
                logger.info(f"Intervención Humana Requerida (HITL) para {conversation_id}")
                hitl_payload = {
